...
public class KeyInputHandler {
	...
	public void onKeyDown(KeyInputEvent event) {
		...
		/*
		 * if (this.test.isKeyDown()) { Minecraft mc = Minecraft.getMinecraft();
		 * net.minecraft.item.ItemStack item = mc.thePlayer.getHeldItem();
		 * System.out.println(item.getDisplayName()); if (item.hasTagCompound()) { if
		 * (item.getTagCompound().hasKey("display", 10)) {
		 * net.minecraft.nbt.NBTTagCompound nbttagcompound =
		 * item.getTagCompound().getCompoundTag("display");
		 * 
		 * if (nbttagcompound.getTagId("Lore") == 9) { net.minecraft.nbt.NBTTagList
		 * nbttaglist1 = nbttagcompound.getTagList("Lore", 8);
		 * 
		 * if (nbttaglist1.tagCount() > 0) { for (int j1 = 0; j1 <
		 * nbttaglist1.tagCount(); ++j1) {
		 * System.out.println(nbttaglist1.getStringTagAt(j1)); } } } } } }/
		 **/
