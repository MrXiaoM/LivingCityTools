...
public class GuiNewIngameMenu extends GuiScreen {
	...
	public void initGui() {
		...
		buttons.add(new GuiMyButton(4, I18n.format("citytools.gamemenu.chatnotice"), this.width / 2 - 150,
				j + i * 4, 120, 16));
		buttons.add(new GuiMyButton(5, I18n.format("gui.advancements"), this.width / 2 - 150, 
				j + i * 5, 120, 16));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		boolean flag = mc.getConnection() != null;
		GlStateManager.pushMatrix();
		float f = 2f;
		GlStateManager.translate(this.width / 2 - 140,
				height / 2 - 75 - mc.fontRenderer.FONT_HEIGHT * (f + 1 + (flag ? 1 : 0)), 0.0F);
		GlStateManager.scale(f, f, f);
		mc.fontRenderer.drawStringWithShadow(I18n.format("citytools.gamemenu.title"), 0, 0, -1);

		GlStateManager.popMatrix();
		mc.fontRenderer.drawStringWithShadow(I18n.format("citytools.gamemenu.subtitle"), this.width / 2 - 140,
				height / 2 - 75 - mc.fontRenderer.FONT_HEIGHT * (1 + (flag ? 1 : 0)), -1);
		if (flag) {
			mc.fontRenderer.drawStringWithShadow(
							mc.getConnection().getPlayerInfoMap().size() == 1 ? I18n.format("citytools.gamemenu.alone")
									: I18n.format("citytools.gamemenu.online",
											mc.getConnection().getPlayerInfoMap().size()),
							this.width / 2 - 140, height / 2 - 75 - mc.fontRenderer.FONT_HEIGHT, -1);
		}
		if (time < 5)
			GlStateManager.popMatrix();
