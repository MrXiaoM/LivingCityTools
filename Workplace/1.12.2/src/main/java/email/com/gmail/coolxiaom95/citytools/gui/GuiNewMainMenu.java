...
public class GuiNewMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	private void drawJumpText() {
			...
			GlStateManager.pushMatrix();
			GlStateManager.translate(this.width / 2 + 90, this.height / 2 - 70.0F, 0.0F);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(this.isBgCustomModExist) {
			top.mrxiaom.custombg.Util.drawBackground(mc);
		}else {
		...
		boolean hovered = mouseX >= this.width - this.fontRenderer.getStringWidth(copyright) - 2
				&& mouseY >= this.height - this.fontRenderer.FONT_HEIGHT - 2 && mouseX < this.width
				&& mouseY < this.height;
		copyright = "§e§l" + (hovered ? "§n" : "") + "Minecraft 由 Mojang AB 版权所有，禁止盗版！";
		this.drawString(this.fontRenderer, copyright,
				this.width - this.fontRenderer.getStringWidth(copyright) - 2,
				this.height - this.fontRenderer.FONT_HEIGHT - 2, -1);
		String networkfix = "§b" + ((mouseX > this.width - mc.fontRenderer.getStringWidth(I18n.format("citytools.networkfix.buttonname")) && mouseX < this.width
				&& mouseY > this.height - (mc.fontRenderer.FONT_HEIGHT*2) - 4 && mouseY < this.height- mc.fontRenderer.FONT_HEIGHT - 4)? "§n":"") 
				+ I18n.format("citytools.networkfix.buttonname");
		this.drawString(this.fontRenderer, networkfix,
				this.width - this.fontRenderer.getStringWidth(networkfix) - 2,
				this.height - (this.fontRenderer.FONT_HEIGHT*2) - 4, -1);
		String s2 = "§a§l欢迎! “" + mc.getSession().getUsername() + "”";
		String s3 = "§r我的世界 1.12.2 §l[ 生存都市客户端 ]";

		this.drawString(this.fontRenderer, s2, 2, this.height - this.fontRenderer.FONT_HEIGHT * 2, -1);
		this.drawString(this.fontRenderer, s3, 2, this.height - this.fontRenderer.FONT_HEIGHT * 1, -1);
	...
	void drawsth() {
		...
		float f = 2F - MathHelper
				.abs(MathHelper.sin(Minecraft.getSystemTime() % 1000L / 1000.0F * (float) Math.PI * 2.0F) * 0.1F);
		GlStateManager.scale(f, f, f);
		this.drawCenteredString(this.fontRenderer, this.splashText, 0, 0, -1);
		GlStateManager.popMatrix();
	}
	...
	public void initGui() {
		...
		if (!this.initialized) {
			...
			mc.getMusicTicker().playMusic(MusicType.MENU);
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (mouseX > this.width - mc.fontRenderer.getStringWidth(copyright) && mouseX < this.width
				&& mouseY > this.height - mc.fontRenderer.FONT_HEIGHT - 2 && mouseY < this.height - 2) {
			mc.displayGuiScreen(new GuiCredit(this));
		}
		else if (mouseX > this.width - mc.fontRenderer.getStringWidth(I18n.format("citytools.networkfix.buttonname")) && mouseX < this.width
				&& mouseY > this.height - (mc.fontRenderer.FONT_HEIGHT*2) - 4 && mouseY < this.height - mc.fontRenderer.FONT_HEIGHT - 4) {
