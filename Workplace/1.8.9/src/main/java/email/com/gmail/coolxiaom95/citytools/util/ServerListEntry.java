...
public class ServerListEntry implements GuiNewList.IGuiListEntry {
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
			...
			field_148302_b.submit(new Runnable() {
				...
				public void run() {
					try {
						...
						ServerListEntry.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_RED + "无法解析域名";
					} catch (Exception var3) {
						ServerListEntry.this.field_148301_e.pingToServer = -1L;
						ServerListEntry.this.field_148301_e.serverMOTD = EnumChatFormatting.DARK_RED + "无法连接到服务器";
					}
				}
			});
		}
		...
		this.mc.fontRendererObj.drawString(this.field_148301_e.serverName, x + 24 + 3, y + 1, 16777215);
		List<String> list = this.mc.fontRendererObj
		...
		if (list.size() > 1) {
			this.mc.fontRendererObj.drawString("§e>> §f§l双击进入§e <<", x + 24 + 3, y + 12, 8421504);
		} else {
			for (int i = 0; i < Math.min(list.size(), 2); ++i) {
				this.mc.fontRendererObj.drawString(list.get(i), x + 24 + 3,
						y + 11 + this.mc.fontRendererObj.FONT_HEIGHT * i, 8421504);
			}
		}
